<?php
/**
	FUNCIONES BASE, NO BORRAR SI QUIERE QUE EL SITIO SIGA FUNCIONANDO
	EDITE BAJO SU PROPIO RIESGO
**/
	...
	function tipodepagina(){ // Identificador del tipo de publicacion
		if(is_home()){ //cuando es home
			...
			if($tipopost=="post"){ $tipo = "articulo"; }else{ $tipo = "articulo postype-".$tipopost; }	
		} elseif(is_category()){//cuando es categoria
			$tipo = "categoria";
		} elseif(is_archive()){ //cuando es archivo
			$tipo = "archivo";
		} elseif(is_search()){ 	//cuando es resultado de busqueda
			$tipo = "busqueda";
	...
	function headElements(){ //incluye elementos basicos del theme en el head
		...
		echo "<link type='text/css' rel='stylesheet' media='screen' href='".$template_url."/css/owl.carousel.min.css'>"."\n"; 		// Owl Carrusel	
	...
	function footerElements(){ 	//incluye elementos basicos del theme en el footer
		...
		$template_url = get_bloginfo('template_url');
		/* SCRIPTS */
	    echo "<script src='".$template_url."/js/jquery-3.3.1.slim.min.js'></script>"."\n"; 	/* ojo, con thrive themes instalado, se debe desinstalar este JQUERY, no permite que se guarden los cambios. */
    	echo "<script src='".$template_url."/js/popper.min.js'></script>"."\n";
    	echo "<script src='".$template_url."/js/bootstrap.min.js'></script>"."\n";
    	/* OWL CARRUSEL */
    	echo "<script src='".$template_url."/js/owl.carousel.min.js'></script>"."\n";
    	/* SCRIPTS BASE */
    	echo "<script src='".$template_url."/js/scripts.js'></script>"."\n";
	}
