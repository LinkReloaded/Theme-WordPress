...
/* ***************************************************************************************/
//   MARK-V							                                           			 //
//   Version: 3.0.8 | 20150311                                               			 //
//   Proyect Author: LinkReloaded - http://www.linkreloaded.net                       	 //
//                                                 			                             //
//                                                                             			 //
...
/* ***************************************************************************************/
	...
	include('functions/function-panelSidebarMenu.php'); 		/* Mostrar o ocultar los menus en el panel de control */
	//include('functions/function-extractoDinamico.php'); 		/* Extracto personalidado */
	include('functions/function-imagenesMiniaturas.php');  		/* Imagenes en miniatura personalizada */
	include('functions/function-shortcode.php');  				/* Atajos de teclado en el sitio */
	//include('functions/function-camposGlobales.php');  		/* Permite editar informacion global del sitio (PREVIA CONFIGURACION) */
	//include('functions/function-cloudTagsCustomPost.php'); 	/* Nube de tags en los post especiales */
	include('functions/function-menuEditable.php'); 			/* Menus editables WORDPRESS*/
	//include('functions/function-paginador1234.php'); 			/* Paginador con numero de listas */
	//include('functions/function-widgetTheme.php'); 			/* Permitir widgets en el theme */
	include('functions/function-panelOpciones.php');  			/* Agrega funcionalidades al panel de control de WP */
...
/* ***************************************************************************************/
//   MARK-V: by: LinkReloaded 				                                             //
/* ****************************************************************************************

/* 	=================================================================================== 		
	VARIABLES GLOBALES 
	=================================================================================== */
	function dato_version(){global $markv_version; $markv_version = "<strong>4SALE-markV</strong><br/>Version: 20150123"; return $markv_version;} //Escribir a la izquierda el nombre de la version.
	function dato_firma(){global $markv_firma; $markv_firma = "&copy;2014, <a href='http://www.linkreloaded.net' target='_blank'>LinkReloaded</a>"; return $markv_firma;} //Escribir a la izquierda la firma del theme.
	...
	function dato_responsivo(){global $dato_responsivo; $dato_responsivo = get_theme_mod('responsivo_dato'); if(!empty($dato_responsivo)){ $dato_responsivo = "si"; } else { $dato_responsivo = "no"; } return $dato_responsivo;}
	...
	/* 	-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 
		...
		<div class="wrap">        
			<p>
				...
				<div class="row">
					<div class="info res<? $dato = dato_responsivo(); if($dato=="si"){?> activado<? };?>"></div>
					<div class="texto left"><strong>THEME RESPONSIVO:</strong><br/>Debe seleccionar si el tema es responsivo en Apariencia/ Personalizar<br/>MUY IMPORTANTE: Para que el tema funcione como responsivo los archivos css deben estar previamente configurados de lo contrario no se ver&aacute; bien.</div>
					<div class="clear"></div>
				</div>
				<div class="row">
...
/* 	=================================================================================== 		
	...
	/* 	-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* 
		...
		<div class="wrap">
			...
			<form method="post" action="">
				<label>Nombre de etiqueta <strong>1</strong> <sup>&lt;?= texto_1(); ?&gt;</sup></label><br/><input name="cod_texto_1" type="text" value="<? echo $markv_texto_1 ?>" size="80"><br/>
	...
	function texto_2(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_2'" );echo $codigo; }
	function texto_3(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_3'" );echo $codigo; }
	function texto_4(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_4'" );echo $codigo; }
	function texto_5(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_5'" );echo $codigo; }
	function texto_6(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_6'" );echo $codigo; }
	function texto_7(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_7'" );echo $codigo; }
	function texto_8(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_8'" );echo $codigo; }
	function texto_9(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_9'" );echo $codigo; }
	function texto_10(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_10'" );echo $codigo; }
	function texto_11(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_11'" );echo $codigo; }
	function texto_12(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_12'" );echo $codigo; }
	function texto_13(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_13'" );echo $codigo; }
	function texto_14(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_14'" );echo $codigo; }
	function texto_15(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_15'" );echo $codigo; }
	function texto_16(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_16'" );echo $codigo; }
	function texto_17(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_17'" );echo $codigo; }
	function texto_18(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_18'" );echo $codigo; }
	function texto_19(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_19'" );echo $codigo; }
	function texto_20(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_20'" );echo $codigo; }
	function texto_21(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_21'" );echo $codigo; }
	function texto_22(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_22'" );echo $codigo; }
	function texto_23(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_23'" );echo $codigo; }
	function texto_24(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_24'" );echo $codigo; }
	function texto_25(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_25'" );echo $codigo; }
	function texto_26(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_26'" );echo $codigo; }
	function texto_27(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_27'" );echo $codigo; }
	function texto_28(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_28'" );echo $codigo; }
	function texto_29(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_29'" );echo $codigo; }
	function texto_30(){ global $wpdb; $nombredb="mark05"; $table_name = $wpdb->prefix.$nombredb; $codigo = $wpdb->get_var("SELECT valor FROM $table_name WHERE dato='texto_30'" );echo $codigo; }
...
/* 	=================================================================================== 		
	...
	add_action( 'customize_register', 'campo_responsivo' );
	...
	if ( !class_exists('myCustomFields') ) {
		class myCustomFields {
			...
			var $customFields =	array(		
				array( "name" => "popuphome_ini", "title" => "Activar / Desactivar Pop Up", "description" => "Click para activar", "type" => "checkbox", "scope" => array( "homepopup" ), "capability" => "edit_posts" ),
				...
				array( "name" => "popuphome_anc", "title" => "Ancho de la imagen", "description" => "Escriba el ancho de la imagen", "type" => "corto", "scope" => array( "homepopup" ), "capability" => "edit_posts" ),
				array( "name" => "popuphome_alt", "title" => "Alto de la imagen", "description"	=> "Escriba el alto de la imagen", "type" => "corto", "scope" =>	array( "homepopup" ), "capability"	=> "edit_posts" ),
				...
				foreach ( $this->customFields as $customField ) {
					if ( current_user_can( $customField['capability'], $post_id ) ) {
						if ( isset( $_POST[ $this->prefix . $customField['name'] ] ) && trim( $_POST[ $this->prefix . $customField['name'] ] ) ) {
							...
							if ( $customField['type'] == "wysiwyg" ) $value = wpautop( $value ); update_post_meta( $post_id, $this->prefix . $customField[ 'name' ], $value ); } else { delete_post_meta( $post_id, $this->prefix . $customField[ 'name' ] );}
